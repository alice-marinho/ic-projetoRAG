...
from backend.src.config.config import MULTILINE_FIELDS, REGEX_FIELDS
class DocsExtractor:
    def __init__(
        self,
    ...
    def extract_fields(self, texto: str)-> list[dict[Any, str | LiteralString]]:
        ...
        logger.debug(f"Garantindo que o diretório exista: {self.raw_data_file}")
        directory = os.path.dirname(self.raw_data_file)
