    ...
    for item in dados_limpos:
        ...
        for campo, valor in item.items():
            if valor and str(valor).strip():
                content_parts.append(f"{campo}: {valor}")
        ...
        documents.append(Document(
            ...
            metadata={
                "source": "componentes_curriculares_extraidos.json",
...
def main():
    # Verifica se precisa processar os documentos (PDF -> JSON -> Chunks)
    if not os.path.exists("dados_limpos.json") or has_docs_changed():
        print("Processando documentos...")
        dados_extraidos = extract_and_save_json()
        dados_limpos = clean_and_save_json(dados_extraidos)

        # Cria chunks do JSON limpo
        json_documents = create_chunks_from_json(dados_limpos)
        json_chunks = split_documents(json_documents)

        # Cria/Carrega o vetorstore
        if not os.path.exists(CHROMA_DIR):
            vectorstore = create_vectorstore(json_chunks)
            print("Base vetorial criada a partir do JSON processado.")
        else:
            # Se já existir, apenas adiciona os novos chunks
            existing_store = load_vectorstore()
            existing_store.add_documents(json_chunks)
            print("Base vetorial atualizada com novos chunks do JSON.")
    else:
